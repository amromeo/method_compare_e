...
load_test_limits <- function(config_file = NULL) {
  if (is.null(config_file)) {
    # Prefer local config when running from repo; fall back to packaged path
    local_path <- file.path("config", "test_limits.json")
    pkg_path <- system.file("app/config/test_limits.json", package = "methodCompare")
    config_file <- if (file.exists(local_path)) local_path else pkg_path
  }
