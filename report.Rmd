...
df_test_long <- data.frame(
  Parameter = c("Test", "Comparison", "Reagent Lot", "Expiration", "Date", "Limit (%)"),
  Value = c(
    params$test,
    paste(params$m1, "vs", params$m2),
    params$reagentLot,
    params$expiration,
    params$date,
    percent(params$limit, accuracy = 0.1)
  )
)
...
# Safety net: ensure table columns match selected method names
if ("X" %in% names(df)) names(df)[names(df) == "X"] <- params$m1
if ("Y" %in% names(df)) names(df)[names(df) == "Y"] <- params$m2
...
if (!is.null(a)) {
  ...
  # Separate the second plot explicitly
  cat("\n\n")  # extra spacing between plots
  par(mfrow = c(1, 1))
  MCResult.plotDifference(data1, plot.type = 4, add.grid = TRUE)
}
```
